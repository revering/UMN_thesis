...
In order to efficiently produce signal events, events are generated with $\epsilon$ set to one and the cross section is multiplied by fixed biasing factors for each \aprime mass. 
...
This bias is rougly proportional to the fraction of muons undergoing \dbrem squared, so the bias factors are selected to ensure that this rate is below 0.23. 

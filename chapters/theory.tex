...
Only the up- and down-flavored quarks are stable, and they make up the 'valence' quarks, which determine the quantum numbers of a particle, of protons and neutrons.

Quarks carry hypercharge, which allows them to interact with photons, W, and Z bosons, as well as color charge, which allows them to interact with gluons. 
Due to the way that gluons carry color charge, the nuclear strong force has an `anti-screening' effect, where virtual gluon pairs cause an increase in the effective interaction strength. 
As a result, the nuclear strong force is weak at small-distance scales and high energy interactions, and diverges at low-energy, large distance scales. 
This causes quark `confinement' where at low temperatures particles carrying color charge cannot be isolated, as quark-antiquark pairs are spontaneously created until all of the quarks are grouped into colorless hadrons or mesons.
...
Three of the leptons carry electric charge and can interact directly with photons: the electron, the muon, and the tau.
...
Because of this, muons produced in CMS collisions generally pass entirely through the detector before decaying, while $\tau$ leptons decay before entering the active detector material.
...
Due to their low interaction rates, the CMS detector is not able to measure neutrinos, and so processes which may cause muons to produce neutrinos with a significant fraction of the initial kinetic energy can potentially imitate signal events.

A graphical representation of the fundamental particles in the SM is shown in \Cref{fig:SM}, along with numerical values of some of their physical properties.
...
These probabilities are measured in terms of a `cross section' which is expressed in terms of area, using a special unit known as a `barn', equal to \SI{10e-28}{\meter\squared}.
The number of expected interactions in an experiment is found by multiplying the cross section of a particular interaction by the `luminosity', measured in events per unit area, which encodes how many potential particle interactions were done.
For collider experiments like CMS, the luminosity is set by the number of particles in the beams and how small the interaction point is.

Developed over several decades through close collaboration between theory and experiment, the SM has been extremely successful at predicting a wide range of observed physics phenomena, an example of which can be seen in \Cref{fig:SMratePreds}, where the observed production cross sections for a wide range of processes are compared with predictions determined from the standard model.
...
A particularly striking example of this comes from the bullet cluster, shown in \Cref{fig:bullet}. 
...
By measuring the pertubation of the CMB and decomposing it into sperical harmonics, the angular power spectrum can be created \Cref{fig:CMBpowerSpectrum}, from which details about the composition of the early universe can be inferred.
...
Under the dark photon kinetic mixing model presented here, muons would primarily produce dark matter in the detector for the A' mass ranges of interest via a process known as \dbrem, where a dark matter particle is emitted from a muon after the muon exchanges a photon with a nucleus in the detector (\Cref{fig:dbrem_feyn}).
...
Instead of the fully generic kinetic mixing presented in \Cref{eq:LDMlagrangian}, the dark photon gauge field can be chosen to contain a current which carries charge proportional to muon number minus tau number, ($L_\mu - L_\tau$)~\cite{neut_trident}.
...
While many such resonances exist in the CMS di-muon spectrum (\Cref{fig:diMuonSpectrum}), this search targets muons originating the decay of Z bosons due to the relatively higher energy of the produced final-state muons and the reduced rates of non-resonant muon pairs near the Z mass.
...
The Z boson resonance in di-muon pairs is primarily produced through the Drell-Yan (DY) process~(\cite{origDYPaper,DYSummary}), in which a pair of quarks in the initial proton-proton collision annihilate into a pair of leptons (\Cref{fig:dyDiagram}).
